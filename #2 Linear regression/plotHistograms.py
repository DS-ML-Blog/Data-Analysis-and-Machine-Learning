...
def plotHistograms(df, nameOfParam):
    ...
    for i, ax in enumerate(axes):
        ...
        meanValue = df.loc[str(2006+i)].loc[:, nameOfParam].mean()
        std = df.loc[str(2006+i)].loc[:, nameOfParam].std()
        ax.plot([meanValue, meanValue], [0, max(ymaxes)], "--", color="red",
                mew=20, ms=30)
        ax.plot([meanValue+std, meanValue+std], [0, max(ymaxes)], "--",
