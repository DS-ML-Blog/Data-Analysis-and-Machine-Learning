...
rename_dict = {'credit.policy': 'decision', 'log.annual.inc': 'log_income', \
        'dti': 'debt_to_income', 'days.with.cr.line': 'days_with_cr_line', \
        'revol.bal': 'amount_unpaid', 'inq.last.6mths': 'recent_inq_num', \
        'delinq.2yrs': 'payment_delay_num', 'not.fully.paid': 'not_fully_paid'}
df.rename(columns = rename_dict, index=str,  inplace = True)

df['purpose'].replace(df['purpose'].unique(), \
                      ['purpose'+str(i) for i in range(len(df['purpose'].unique()))], \
                      inplace = True)
...
# 4. Zależność między fico i days_with_cr_line
fig = plt.figure(figsize = (9,9))
ax = fig.add_subplot(111)
...
# 6. Scatterplot przychodów i wielkości raty
fig = plt.figure(figsize = (9,9))
ax = fig.add_subplot(111)
